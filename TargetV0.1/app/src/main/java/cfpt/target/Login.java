    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                task.SetRequete("SELECT `prenom` FROM `test` WHERE `prenom`=\""+mEmail+"\"");
                task.Connect();
                if (!task.getListeRes().isEmpty()){
                    task.SetRequete("SELECT `mdp` FROM `test` WHERE `prenom`=\""+mEmail+"\"");
                    task.Connect();
                    if(task.getListeRes().get(1).equals(mPassword)){
